...
file_path = "D:/vector-tile-program/9_cities_total_road_network"
out_path = "D:/vector-tile-program/9_cities_total_road_network_out"
...
tipp_rules = {
    "LandUseArea":
        [
            '''tippecanoe -e {0}/china_landusearea -l china_landusearea -F -pC -Z8 -z14 -g 1'''
        ],
    "LandUseLine":
        [
            '''tippecanoe -e {0}/china_landuseline -l china_landuseline -F -pC -Z8 -z14 -g 1'''
        ],
    "Road":
        [
            '''tippecanoe -e {0}/china_way -l china_way -F -pC -j '{"*":["all",["in","FC",1,2]]}' -Z8 -z9 -g 0.6''',
            '''tippecanoe -e {0}/china_way -l china_way -F -pC -j '{"*":["all",["in","FC",1,2,3]]}' -Z10 -z11 -g 0.8''',
            '''tippecanoe -e {0}/china_way -l china_way -F -pC -j '{"*":["all",["in","FC",1,2,3,4]]}' -Z12 -z13 -g 1''',
            '''tippecanoe -e {0}/china_way -l china_way -F -pC -Z14 -z14'''
        ],
    "SettlementArea":
        [
            '''tippecanoe -e {0}/china_settlementarea -l china_settlementarea -F -pC -Z14 -z14'''
        ],
    "SettlementLine":
        [
            '''tippecanoe -e {0}/china_settlementline -l china_settlementline -F -pC -Z14 -z14'''
        ],
    "Railway":
        [
            '''tippecanoe -e {0}/china_railway -l china_railway -F -pC -Z6 -z14 -as '--simplification=10' -pk'''
        ],
    "poi_cms":
        [
            '''tippecanoe -e {0}/china_poi_10 -l china_poi -j '{ "*": ["all",[ "in", "Type","250102","290200","290201","290202"]] }' -F -pC -Z10 -z10''',
            '''tippecanoe -e {0}/china_poi_11 -l china_poi -j '{ "*": ["all",[ "in", "Type","210201","210202","241201","250200"]] }' -F -pC -Z11 -z11''',
            '''tippecanoe -e {0}/china_poi_12 -l china_poi -j '{ "*": ["all",[ "in", "Type","180101","210203","210103","210102","210104","290106"]] }' -F -pC -Z12 -z12''',
            '''tippecanoe -e {0}/china_poi_13 -l china_poi -j '{ "*": ["all",[ "in", "Type","190101","220201","250301","250302","250303","250300","250400","290203"]] }' -F -pC -Z13 -z13''',
            '''tippecanoe -e {0}/china_poi_14 -l china_poi -j '{ "*": ["all",[ "in", "Type","100102","180200","190100","210100","220200","230101","241300","241202"]] }' -F -pC -Z14 -z14''',
            '''tippecanoe -e {0}/china_poi_15 -l china_poi -j '{ "*": ["all",[ "in", "Type","100105","160100","160301","180202","180601","180602","180603","210204","220300","220301","220302","230103","230104","240600","240700","241203","250101","250100","290108"]] }' -F -pC -Z15 -z15'''
        ],
    "CrossRadiuscp": ['''tippecanoe -e {0}/cross_radiuscp -l cross_radiuscp -F -pC -Z14 -z14'''],
    "RoadFurnitureJY": ['''tippecanoe -e {0}/road_furniturejy -l road_furniturejy -F -pC -Z14 -z14'''],
    "LD_NAV_LINK": [
        '''tippecanoe -e {0}/blue_way -l blue_way -F -pC -Z8 -z9 -g 0.6''',
        '''tippecanoe -e {0}/blue_way -l blue_way -F -pC -Z10 -z11 -g 0.8''',
        '''tippecanoe -e {0}/blue_way -l blue_way -F -pC -Z12 -z13 -g 1''',
        '''tippecanoe -e {0}/blue_way -l blue_way -F -pC -Z14 -z14'''
    ],
}
...
def log(text_log):
    ...
    print("\n " + text_log + "|" + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), file=data)
    data.close()
...
def exportsh(name, files, tipp):
    ## [0].replace("{0}", out_path)
    index = 0
    for ti in tipp:
        index += 1
        # print("\r \t " + ti.replace("{0}", out_path) + " " + str(files))
        data = open(file_path + "/" + name + "_" + str(index) + ".sh", "w+", encoding="utf-8")
        print(ti.replace("{0}", out_path) + " " + str(files), file=data)
        data.close()
...
def run_vector_production():
    ...
    for c in commands:
        ...
        if c.lower().find('.sh') > -1:
            ...
            thread = Thread(target=base, args=("sh " + str(c),))
            thread.daemon = True
            thread.start()
            threads.append(thread)

            command_array.append("sh "+str(c))
            print(str(command_index)+":"+str(c))
            command_index += 1

        # process_bar(round(100 / len(commands), 6) * command_index / 100, start_str='[sh processing]', end_str=' ',total_length=50)

    # log(command_array)
    #ls = " && ".join(command_array)
    #print(ls)
    # os.system(";".join(command_array)) subprocess
...
def shp_to_json(path):
    ...
    for p in path:
        ...
        index = index + 1
    log("convert [" + str(index) + "] shp file")
    del path
...
def init_run():
    log("*************************************")
    tippecanoe_v = os.system("tippecanoe -v")
    gdal_v = os.system("gdalinfo --version")
    log(str(tippecanoe_v) + "|" + str(gdal_v))
    log("*************************************")
    log("*************** 作业开始 *************")
    log("*************************************")
...
if __name__ == '__main__':
    if sys.argv[1:]:
        if len(sys.argv) < 2:
            print("No working path,python shp_compile.py [input dir path] [output dir path]")
